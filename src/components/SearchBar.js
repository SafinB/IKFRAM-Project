...
const SearchBar = ({ setExercises, bodyPart, setBodyPart }) => {
  ...
  useEffect(() => {
    const fetchExercisesData = async () => {
      const bodyPartsData = await api('https://exercisedb.p.rapidapi.com/exercises/bodyPartList', exerciseOptions);
  ...
  const handleSearch = async () => {
    if (search) {
      const exercisesData = await api('https://exercisedb.p.rapidapi.com/exercises', exerciseOptions);
    ...
    <Stack alignItems="center" mt="37px" justifyContent="center" p="20px">
      ...
      <Box position="relative" mb="72px">
        <TextField
          ...
          type="text"
        />
        <Button className="search-btn" sx={{ bgcolor: '#19A7CE', color: '#fff', textTransform: 'none',
